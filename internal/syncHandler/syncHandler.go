...
func (s SyncHandler) Sync() {
	...
	for _, mapping := range allMappings {
		for _, season := range allSeasons {
			if mapping.SeasonId == season.Id {
				...
				if !ok {
					update = Update{AnimeId: mapping.AnimeId, Status: animeList.Planning, Progress: 0, New: false, LastWatched: season.LastViewedAt}
				}
	...
	for _, update := range updates {
		s.log.Info("Updating anime", slog.String("animeId", update.AnimeId), slog.String("status", fmt.Sprint(update.Status)), slog.Int("progress", update.Progress), slog.Bool("new", update.New), slog.Time("LastViewedAt", update.LastWatched))
	}
}
