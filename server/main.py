...
from config import MARK_UNWATCHED_EPISODES_AS_PLANNING, PLEX_SERVER_URL, ANILIST_TOKEN, TIME_UNTIL_DROPPED, TIME_UNTIL_PAUSED
...
from services.animeListServices.anilistService import AnilistService, Anime
...
def create_mapping_for_anime_series(series: List[PlexAnime]):
    for anime in series:
        ...
        mapping = mapping_service.get_mapping_by_tvdb_id(anime.tvdb_id, anime.season_number)

        # If we didn't find one we should check for the wildcard listing
        if len(mapping) == 0:
            mapping = mapping_service.get_mapping_by_tvdb_id(anime.tvdb_id, "*")
...
def get_watch_status(plex_anime: PlexAnime, mapping: TvdbToAnilistMapping, anime_list_anime: AnimeListAnime):
    ...
    if anime_list_anime is None:
        anilist_anime = anilist_service.get_anime(mapping.anilist_id)
    ...
    if watch_episodes_unchanged and plex_anime.episodes_watched > 0 and days_since_last_viewing is not None and days_since_last_viewing > TIME_UNTIL_DROPPED:
        return 5

    # Paused
    # Needs to be updated depending on how long it's been since the last update and if the current status is Current
    if watch_episodes_unchanged and plex_anime.episodes_watched > 0 and days_since_last_viewing is not None and days_since_last_viewing > TIME_UNTIL_PAUSED:
        return 4
    ...
    # Planning
    if MARK_UNWATCHED_EPISODES_AS_PLANNING and plex_anime.episodes_watched == 0:
        return 3


def process_wildcard_mapping(series: List[PlexAnime], anime_list: AnimeList):
    ...
    wildcard_mappings: List[TvdbToAnilistMapping] = mapping_service.get_mapping_by_tvdb_id(
    ...
    for wildcard_mapping in wildcard_mappings:
        ...
        if not MARK_UNWATCHED_EPISODES_AS_PLANNING and imutable_plex_anime.episodes_watched == 0:
            return
        ...
        if wildcard_mapping.season_length is not None:
            ...
            end_index = min(wildcard_mapping.episode_start + wildcard_mapping.season_length, len(plex_anime.episodes))
        ...
        watch_status = get_watch_status(plex_anime, wildcard_mapping, list_anime)
        ...
        if list_anime is None:
            anilist_service.update_anime(wildcard_mapping.anilist_id,
...
def process_series(series: List[PlexAnime], anime_list: AnimeList):
    process_wildcard_mapping(series, anime_list)

    for immutable_plex_anime in series:
        # We keep track of wildcard mappings to suppress "no mappings" logs
        wildcard_mappings: List[TvdbToAnilistMapping] = mapping_service.get_mapping_by_tvdb_id(series[0].tvdb_id, "*")

        mappings: List[TvdbToAnilistMapping] = mapping_service.get_mapping_by_tvdb_id(
        ...
        for mapping in mappings:
            ...
            # We can skip if no episodes have been watched and we aren't setting shows as planning
            if not MARK_UNWATCHED_EPISODES_AS_PLANNING and plex_anime.episodes_watched == 0:
                continue

            list_anime = anime_list.get_anime(mapping.anilist_id)
            watch_status = get_watch_status(plex_anime, mapping, list_anime)
            # If we don't have a watch status we can skip this one
            if watch_status is None:
                continue
            ...
            # The anime isn't on the list so we need to add it
            if list_anime is None:
                anilist_service.update_anime(mapping.anilist_id, plex_anime.episodes_watched,
...
def main():
    ...
    for series in all_anime:
        if series[0].tvdb_id != "71361":
            continue
        create_mapping_for_anime_series(series)
        process_series(series, anime_list)
